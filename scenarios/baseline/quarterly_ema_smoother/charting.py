...
OUTPUT_DIR = '/home/realdomarp/PYMC/FACTOR ROTATION/scenarios/baseline/quarterly_ema_smoother/output'
...
try:
    first_valid = results['regime'].dropna().iloc[0]
...
axes[0].set_title('Equity Curve - Quarterly EMA + Smoothed HMM Strategy')
...
prob_bull_vals = results['prob_bull'].fillna(0.5).values
axes[3].plot(results.index, prob_bull_vals, color='purple', linewidth=1, alpha=0.7, label='prob_bull')
axes[3].plot(results.index, results['regime'].values, color='blue', linewidth=1, alpha=0.5, linestyle='--', label='regime')
axes[3].axhline(y=0.64, color='red', linestyle='--', linewidth=1, label='Fast exit (0.64)')
axes[3].axhline(y=0.47, color='orange', linestyle='--', linewidth=1, label='Slow exit (0.47)')
