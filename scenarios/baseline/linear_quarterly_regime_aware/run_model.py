...
Risk Aversion: Regime-Aware (0.5 for bull, 1.0 for bear)
...
- Dynamic risk aversion based on HMM regime
"""
...
OUTPUT_DIR = '/home/realdomarp/PYMC/FACTOR ROTATION/scenarios/baseline/linear_quarterly_regime_aware/output'

# Risk aversion by regime (determined from HMM)
# Assume: state 0 = bull, state 1 = transition, state 2 = bear
RA_BULL = 0.5    # More aggressive in bull markets
RA_BEAR = 1.0    # More conservative in bear markets

# Create output directory
os.makedirs(OUTPUT_DIR, exist_ok=True)

# === LOAD DATA ===
print("=== Baseline Linear Model (Quarterly, Regime-Aware RA) ===")
print("Loading data...")
...
n = len(indices)
print(f"\nRunning backtest with {n} quarterly iterations (Regime-Aware RA)...")
...
for idx, i in enumerate(indices):
    ...
    # ============ DETERMINE RISK AVERSION BASED ON REGIME ============
    # State 0 = bull (low volatility), State 2 = bear (high volatility)
    if hmm_state_for_X_test == 2:
        current_ra = RA_BEAR
    else:
        current_ra = RA_BULL
    ...
    # ============ PORTFOLIO CONSTRUCTION WITH REGIME-AWARE RA ============
...
# RA usage summary
ra_bull_count = sum(1 for r in ra_used if r == RA_BULL)
ra_bear_count = sum(1 for r in ra_used if r == RA_BEAR)
print(f"\nRA Usage: {ra_bull_count} bull periods, {ra_bear_count} bear periods")
...
# Also save RA history for analysis
ra_df = pd.DataFrame({'date': dates, 'hmm_state': hmm_states, 'risk_aversion': ra_used})
...
charting.plot_all_charts(
    ...
    title_prefix='Linear Quarterly Regime-Aware'
)
